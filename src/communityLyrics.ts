...
const time = z.number().min(0).max(86400);
...
export function separateVocals(input: string): { lead: string; background: string[] } {
    ...
    for (const char of input) {
        ...
        if (char === ')' && depth > 0) {
            if (--depth === 0 && phrase.trim()) {
                background.push(normalize(phrase));
            }
...
export function validateCommunityLyrics(input: unknown, source: LineSyncedLyrics): SyllableSyncedLyrics {
    ...
    const matchesBackground = (given: string[], expected: string[]) => {
        ...
        return given.every(s => {
            const at = expected.indexOf(s, cursor);
            cursor = at + 1;
    ...
    return {
        ...result,
        SongWriters: source.SongWriters ?? [],
        Community: true
    };
}
