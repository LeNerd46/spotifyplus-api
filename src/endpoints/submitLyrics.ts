...
export const SubmitLyrics = async (c: Context<{ Bindings: Env }>) => {
    ...
    if (!/^[a-zA-Z0-9]{22}$/.test(id)) return c.json({ error: 'Invalid Spotify track ID' }, 400);
    let input: unknown;
    ...
    if (!sourceJson) return c.json({ error: 'Load the original line lyrics before submitting' }, 409);
    const source = JSON.parse(sourceJson) as LineSyncedLyrics;
    if (source.Type !== 'Line') return c.json({ error: 'Community sync requires original line lyrics' }, 409);
    let lyrics;
    try { lyrics = validateCommunityLyrics(input, source); }
    catch (error) { return c.json({ error: error instanceof Error ? error.message : 'Invalid lyrics' }, 400); }
    // A separate preferred key protects submissions from in-flight provider fetches.
    // Retain the source for validation of retries and future corrections.
